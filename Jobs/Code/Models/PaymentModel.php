...
class PaymentModel extends BaseModel {
    ...
    public function updatePayment($payment, $status, $stage) {
        ...
        if ($status) {
            ...
            $job = $factory->getRecord('#__job_jobs', 'jj', array('jj.id = :id'), array('id' => $payment->item_id));
            $package_price = $factory->getRecord('#__job_packages_prices', 'jpp', array('jpp.id = :id'), array('id' => $job->package_price_id));

            $data->payment_expiry = $this->getExpiryDate($job->payment_expiry , $package_price->no_of_days); // (time() > strtotime($job->payment_expiry));
        }
        $data->payment_date = $payment->item_id;

        $factory->saveRecord('#__job_jobs', $data);
    }
    ...
    public function getDataObject($payment) {
        ...
        $job = $factory->getRecord('#__job_jobs', 'jj', array('jj.id = :id'), array('id' => $payment->item_id));
        $user = $factory->getRecord('#__users_users', 'uu', array('uu.id = :id'), array('id' => $job->created_by));

        $tmp_array['user'] = $user;
        $tmp_array['job'] = $job;
        $tmp_array['payment'] = $payment;
        $tmp_array['url']['job'] = $this->generateUrl('jobs.jobs', null, 0);
